...
```shell
python -m pylint intg-powerview
python -m flake8 intg-powerview --count --show-source --statistics
python -m isort intg-powerview/. --check --verbose 
python -m black intg-powerview --check --verbose --line-length 120
```
...
### Format Code
```shell
python -m black intg-powerview --line-length 120
```
